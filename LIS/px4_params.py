...
class DroneController:
    ...
    async def prime_attitude_rate_setpoint(self, seconds=1.5, dt=0.05):
        print(f"-- Priming attitude-rate setpoint for {seconds:.1f}s")
        n = int(seconds / dt)
        for _ in range(n):
            await self.drone.offboard.set_attitude_rate(AttitudeRate(0.0, 0.0, 0.0, 0.0))
            await asyncio.sleep(dt)

    async def send_attitude_rate_for(self, roll_rate, pitch_rate, yaw_rate, thrust, seconds, dt=0.05):
        n = int(seconds / dt)
        for _ in range(n):
    ...
    async def get_quick_state(self, timeout_s=0.6):
        """Read one-shot in_air/armed state with short timeout."""
        in_air = None
        armed = None
        try:
            async with asyncio.timeout(timeout_s):
        ...
        try:
            async with asyncio.timeout(timeout_s):
                async for v in self.drone.telemetry.armed():
                    armed = bool(v)
                    break
        except TimeoutError:
            pass
    ...
    async def safe_shutdown(self, landed_timeout_s=12.0, disarm_retries=4, retry_interval_s=0.6):
        """
        Exit offboard safely:
        1) wait landed with timeout
        2) disarm with retries while checking armed flag
        3) best-effort stop offboard (ignore mode-denied)
        """
        landed = await self.wait_until_landed(timeout_s=landed_timeout_s)
        if not landed:
            print("-- Landed not confirmed, continue disarm retries anyway")

        for i in range(disarm_retries):
            in_air, armed = await self.get_quick_state()
            if armed is False:
            ...
            try:
                ...
                print(f"Disarm attempt {i + 1}/{disarm_retries} failed: {e}")
                if i + 1 < disarm_retries:
                    await asyncio.sleep(retry_interval_s)

        # Stop offboard at the end. PX4 may deny mode switch on some configs.
        try:
